...
seed_df <- read_csv("seedSearchOutput/seedSearch_2025-01-29 00:05:00.499335.csv") %>% 
...
lam_df = expand.grid(asp.mag = seq(7,15,1),
                     clim = seq(-50,50,1),
                     s1 = NA, s2 = NA, pop=NA)
...
asp.mag.i = 2
asp.effects = c(-1,1)
n.subpops = as.numeric(length(asp.effects))
subpop.output = T
seeds <- seed_df %>% filter(asp.effects=="-1,1" & asp.mag==asp.mag.i)
seeds <- 6210

df <- subpopsThruTime_fun(burn.in.yrs = burn.in.yrs,
                          sim.yrs = sim.yrs,
                          clim.sd = 3,
                          move.rate = 0.05,
                          asp.mag = asp.mag,
  ...
  mutate(year = year-burn.in.yrs) 

ggplot(filter(df,year>=0),aes(x=year))+
  geom_line(aes(y=sub1.size,color="Subpopulation A"),linewidth=1.5)+
  geom_line(aes(y=sub2.size,color="Subpopulation B"),linewidth=1.5)+
  labs(x="Year",y="Size through time")+
  theme_bw(base_size = 16)+
