...
out_df <- foreach(am.i = 1:length(asp.mags), .combine=rbind, .packages=c("tidyverse","EnvStats","Matrix")) %:%
  foreach(clim.i = 1:length(clims), .combine=rbind) %:%
  foreach(mr.i = 1:length(move.rates), .combine=rbind) %dopar% {
    ...
    seeds_surv_subs <- glm(survival_status~size + (region.clim + asp.mag*asp.effect) + (region.clim + asp.mag*asp.effect)^2, family="binomial", data=seed_df)
    seeds_surv_pooled <- glm(survival_status~size + region.clim + region.clim^2, family="binomial", data=seed_df)
    ...
    juv_surv_subs <- glm(survival_status~size + (region.clim + asp.mag*asp.effect) + (region.clim + asp.mag*asp.effect)^2, family="binomial", data=juv_surv_df)
    juv_surv_pooled <- glm(survival_status~size + region.clim + region.clim, family="binomial", data=juv_surv_df)
    ...
    juv_grow_subs <- glm(growth_status~size + (region.clim + asp.mag*asp.effect) + (region.clim + asp.mag*asp.effect)^2, binomial( link = "logit" ), data=juv_grow_df)
    juv_grow_pooled <- glm(growth_status~size + region.clim + region.clim^2, binomial( link = "logit" ), data=juv_grow_df)
    ...
    flrg_surv_subs <- glm(survival_status~size + (region.clim + asp.mag*asp.effect) + (region.clim + asp.mag*asp.effect)^2, family="binomial", data=flrg_df)
    flrg_surv_pooled <- glm(survival_status~size + region.clim + region.clim^2, family="binomial", data=flrg_df)
    ...
    seed_df <- expand_grid(clim_df,
                           ...
                           asp.mag = am,
                           size = 1) %>% 
      mutate(seed_surv_subs = stats::predict(seeds_surv_subs,newdata = .,type = "response"),
             seed_surv_pooled = stats::predict(seeds_surv_pooled,newdata = .,type = "response")) %>% 
      select(-size)
