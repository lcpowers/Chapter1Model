...
for(am.i in 1:length(asp.mags)){
  ...
  for(clim.i in 1:length(clims)){
    ...
    for(mr.i in 1:length(move.rates)){
      ...
      juv_surv_pooled_lm <- glm(survival_status~size + region.clim + region.clim, family="binomial", data=juv_surv_lm_df)
...
all_out_df2 <- data.table::rbindlist(out_list)

lambda_df <- all_out_df2 %>%
  unique() %>% 
