...
asp.effects <-
  list(
    c(-2, 1), # Simple
    c(-2,-1, 1), # More pops
    c(-2,-1, 1,2) # More pops more dramatic
)
...
seed_climsd_df <- read_csv("seedSearchOutput/maxLam_2025-02-11 13:20:30.csv") %>% 
...
seed_search <- foreach(r = 1:reps, .combine=rbind, .packages=c("tidyverse","EnvStats")) %dopar% {
  for(i in 1:nrow(seed_climsd_df)){
    ...
    asp.effects.i <- strsplit(seed_climsd_df$asp.effects[i],",") %>% unlist()
