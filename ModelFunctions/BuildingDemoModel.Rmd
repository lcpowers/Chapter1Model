...
seed_df_refined <- 
  foreach(r = 1:reps, .combine=rbind) %:%
   foreach(asp.mag.i = asp.mags, .combine=rbind, .packages=c("tidyverse","EnvStats")) %:% 
    ...
    if(asp.mag.i %% 2 == 1){
      seeds <- seed_df_coarse$seeds[seed_df_coarse$asp.mag == (asp.mag.i-1) & seed_df_coarse$clim.sd == (clim.sd.i - 1)]
    } else {
      seeds <- seed_df_r1$seeds[seed_df_r1$asp.mag == asp.mag.i & seed_df_r1$clim.sd == clim.sd.i]
    }
    ...
    while(x>0.000005){
      ...
      # If this is negative, then lt lam is too high and seed # needs to decrease
      lam.diff = 1.0005 - lt.lam
            
      if(lam.diff < 0){ 
          seeds <- seeds - ceiling(lt.lam*100)
          } else if(lam.diff > 0) {
            seeds <- seeds + ceiling(lt.lam*100)
            } else if(lam.diff == 0){
              seeds <- seeds
